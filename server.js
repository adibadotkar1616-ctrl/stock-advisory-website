...
async function api(req,res){
  ...
  if(p==='/api/dashboard'&&req.method==='GET'){
    const [plans,research,calls,recs,portfolio,payments,notifications,kyc]=await Promise.all([
      ...
      pool.query('SELECT id,user_id AS "userId",plan_id AS "planId",amount,status,method,created_at AS "createdAt" FROM payments WHERE user_id=$1 ORDER BY id DESC',[u.id]),
  ...
  if(p==='/api/admin'&&req.method==='GET'){
    ...
    const [users,plans,research,calls,recs,portfolio,payments,notifications,kyc]=await Promise.all([
      ...
      pool.query('SELECT id,user_id AS "userId",plan_id AS "planId",amount,status,method,created_at AS "createdAt" FROM payments ORDER BY id DESC'),
  ...
  if(p==='/api/plan'&&req.method==='POST'){
    ...
    await pool.query('UPDATE users SET plan_id=$1 WHERE id=$2',[planId,u.id]);
    await pool.query(
  ...
  if(p==='/api/admin/payment'&&req.method==='POST'){
    const x=await body(req);
    const r=await pool.query('SELECT * FROM payments WHERE id=$1',[Number(x.id)]);
    if(!r.rowCount)return send(res,404,{error:'Payment not found.'});
    const status=x.status||'paid';
    await pool.query('UPDATE payments SET status=$1 WHERE id=$2',[status,Number(x.id)]);
    if(status==='paid')
      await notify(r.rows[0].user_id,'Payment confirmed','Your subscription payment has been marked as paid.');
    return send(res,200,{ok:true});
  }
