    ...
    <main class="page">
      <div class="card auth-card">
        ...
        <form onsubmit="submitAuth(event, '${mode}')">
          ...
          ${isLogin ? `<button type="button" class="secondary" onclick="forgotPassword()">Forgot Password?</button>` : ''}
        </form>
...
async function forgotPassword() {
  ...
  if (!email) return;
  try {
    const result = await api('/api/forgot-password', { method: 'POST', body: { email } });
    alert(result.message || 'If an account exists, a reset link has been sent.');
  } catch (err) {
    alert(err.message);
  }
}
...
async function submitAuth(e, mode) {
  ...
  try {
    ...
    if (result.emailVerificationRequired) {
      alert(result.message || 'Please check your email to verify your account.');
      auth('login');
      return;
    }
