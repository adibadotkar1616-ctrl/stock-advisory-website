...
function showOtpVerification(email, message) {
  app.innerHTML = `
    ${nav()}
    <main class="page">
      <div class="card auth-card">
        <h2>Verify Your Email</h2>
        <p class="muted">${esc(message || 'We sent a 6-digit verification code to your email.')}</p>
        <p><strong>${esc(email)}</strong></p>
        <form onsubmit="submitOtp(event)">
          <label>6-Digit OTP</label>
          <input type="text" name="otp" inputmode="numeric" autocomplete="one-time-code" maxlength="6" pattern="[0-9]{6}" required placeholder="Enter 6-digit OTP" />
          <button class="primary" type="submit">Verify Email</button>
        </form>
        <div class="auth-switch">
          <button onclick="resendOtp('${esc(email)}')">Resend OTP</button>
          <button onclick="auth('login')">Back to Login</button>
        </div>
      </div>
    </main>`;
}
...
async function submitOtp(e) {
  ...
  x.email=window.vertexOtpEmail||'';
  try {
    const result=await api('/api/verify-otp',{method:'POST',body:x});
...
async function resendOtp(email) {
  try {
    const result=await api('/api/resend-otp',{method:'POST',body:{email}});
...
async function submitAuth(e, mode) {
  ...
  const x = Object.fromEntries(new FormData(form).entries());
  try {
    const result = await api(mode === 'login' ? '/api/login' : '/api/register', {method:'POST',body:x});
    if(mode==='register' && result.emailVerificationRequired){
      window.vertexOtpEmail=x.email;
      showOtpVerification(x.email,result.message);
      return;
    }
