...
async function submitAuth(e, mode) {
  ...
  try {
    ...
    if (mode === 'register') {
      // The PostgreSQL/Brevo backend creates the account first and sends a
      // verification email. Do NOT create a client session until verification.
      if (!result.token || result.emailVerificationRequired || result.verificationRequired) {
        showVerificationPending(x.email, result.message || 'Your account was created. Please check your email and click the verification link before logging in.');
        return;
      }
    }
  ...
  app.innerHTML = `
    ...
    <main class="page">
      <div class="card auth-card" style="text-align:center;">
        ...
        <p class="muted" style="line-height:1.7;">${message}</p>
        <p><strong>${escapeHtml(email || '')}</strong></p>
        <div style="margin-top:22px;display:flex;gap:10px;justify-content:center;flex-wrap:wrap;">
          <button class="primary" onclick="auth('login')">Go to Client Login</button>
          <button onclick="auth('register')">Back to Registration</button>
        </div>
        <p class="muted" style="margin-top:18px;font-size:13px;">If you don't see the email, check your Spam/Junk or Promotions folder.</p>
      </div>
    </main>
  `;
}
